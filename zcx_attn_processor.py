...
def customized_scaled_dot_product_attention(query, key, value, weight_matrix=None, disp_coc=None,
    ...
    if weight_matrix is None: # Falling to vanilla attention
        ...
        # attn_weight_qk[.repeat(1,attn_weight_qk.shape[1],1,1)] = -1e8
        # attn_weight_manual_normed = attn_weight_manual/torch.sum(attn_weight_manual,dim=-2,keepdims=True)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.segment_num <= 0:
            ...
            # (disp_coc[:,[0], :] - disp_coc[:,0,:,None] < 0).unsqueeze(1)
        else:
            occ_map = []

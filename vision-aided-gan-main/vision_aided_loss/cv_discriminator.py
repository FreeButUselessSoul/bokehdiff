...
class SimpleD(nn.Module):
    def __init__(self, in_ch=768, out_ch=256, out_size=8, num_classes=0, activation=nn.LeakyReLU(0.2, inplace=True)):
        super().__init__()
...
class unet_disc(torch.nn.Module):
    def __init__(self, in_ch=3, out_ch=16, num_classes=0, activation=nn.LeakyReLU(0.2, inplace=True)):
        super().__init__()
        ...
        self.uppool = nn.Upsample(scale_factor=2, mode='nearest')
        for i in range(6):
            encoder_inch = cr_in_ch if i>0 else in_ch
            self.encoder.append(nn.Sequential(
                nn.Conv2d(encoder_inch, cr_in_ch*2, kernel_size=3, stride=1, padding=1),
                nn.InstanceNorm2d(cr_in_ch*2),
                nn.Conv2d(cr_in_ch*2, cr_in_ch*2, kernel_size=3, stride=1, padding=1),
                ))
        
            
            self.decoder.append(nn.Sequential(
                nn.Conv2d(cr_in_ch*2, cr_in_ch, kernel_size=3, stride=1, padding=1),
                nn.InstanceNorm2d(cr_in_ch),
                nn.Conv2d(cr_in_ch, cr_in_ch, kernel_size=3, stride=1, padding=1), 
                activation,
            ))

            cr_in_ch *= 2
        self.predictor_mid = nn.Sequential(
            nn.Conv2d(cr_in_ch, 64, kernel_size=3, stride=1, padding=1),
            activation,
            spectral_norm(nn.Conv2d(64, out_ch, kernel_size=3, stride=1, padding=1)),
        )
        self.predictor_end = nn.Conv2d(out_ch, 1, kernel_size=3, stride=1, padding=1)
        self.to_out = spectral_norm(nn.Linear(out_ch, 1))
        self.embed = None
        if num_classes > 0:
            self.embed = nn.Embedding(num_classes, out_ch)
    ...
    def forward(self, x, c):
        ...
        for i in range(6):
            ...
            x = self.downpool(x)
        mid_pred = torch.mean(self.predictor_mid(x), dim=(2,3), keepdim=True)
        for i in range(5, -1, -1):
            x = self.uppool(x)
            x = self.decoder[i](residual_list[i] + x)
        end_pred = self.predictor_end(x)
        out = self.to_out(mid_pred) + torch.mean(end_pred, dim=(2,3), keepdim=True)
        if self.embed is not None:
            out += torch.sum(self.embed(c) * x, 1, keepdim=True)
        return out

class Discriminator(torch.nn.Module):
    def __init__(self, cv_type, output_type=None, loss_type=None, diffaug=False, device='cpu', create_optim=False, num_classes=0, activation=nn.LeakyReLU(0.2, inplace=True), **kwargs):
        super().__init__()
        if output_type is None:
            output_type = '+'.join(['conv_multi_level']*len(cv_type.split('+')))
        ...
        print(f"Got {self.num_models} CV backbone models.")

        def get_decoder(cv_type, output_type):
            if 'clip' in cv_type:
                # if 'conv_multi_level' in output_type:
                #     decoder = MultiLevelDViT(level=3, in_ch1=2048, in_ch2=512, out_ch=256, num_classes=num_classes, activation=activation)
                # else:
                decoder = MLPD(in_ch=2048, out_ch=256, num_classes=num_classes, activation=activation)
            if 'convnext' in cv_type:
                decoder = MLPD(in_ch=1024, out_ch=256, num_classes=num_classes, activation=activation)
            ...
            if 'vgg' in cv_type:
                decoder = SimpleD(in_ch=512, out_ch=256, out_size=8, num_classes=num_classes, activation=activation)
            ...
            if 'zcx' in cv_type:
                decoder = unet_disc(in_ch=3, out_ch=1, num_classes=num_classes, activation=activation)
            return decoder

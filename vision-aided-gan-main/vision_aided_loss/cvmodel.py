...
class CLIP(torch.nn.Module):
    ...
    def __init__(self, cv_type='adv'):
        ...
        self.cv_type = cv_type
        self.model, _ = clip.load("RN50", jit=False, device='cpu')
    ...
    def forward_custom(self, x):
        ...
        x = torch.mean(x.flatten(2,3), dim=2)
        return x
    ...
    def __call__(self, x):
        ...
        if 'conv_multi_level' in self.cv_type:
            ...
            # x[0] = x[0][:, 1:, :].permute(0, 2, 1).reshape(-1, 480, 32, 32).float()
            # x[1] = x[1][:, 1:, :].permute(0, 2, 1).reshape(-1, 480, 32, 32).float()
            # x[2] = x[2].float()
        else:
...
class DINO(torch.nn.Module):
    ...
    def __call__(self, x):
        x = x*0.5+0.5 #   F.interpolate(x*0.5+0.5, size=(224, 224), mode='area')
        x = x - self.image_mean[:, None, None].to(x.device)
        x /= self.image_std[:, None, None].to(x.device)
        ...
        if 'conv_multi_level' in self.cv_type:
            ...
            x[0] = x[0][:, 1:, :].permute(0, 2, 1).reshape(-1, 768, 32, 32).contiguous()
            x[1] = x[1][:, 1:, :].permute(0, 2, 1).reshape(-1, 768, 32, 32).contiguous()
            x[2] = x[2][:, 0, :]
        else:
            x = self.model(x)
...
class ConvNext(torch.nn.Module):
    ...
    def __call__(self, x):
        ...
        x = x.permute(0,2,3,1)# .flatten(-3,-1)#
        return x

class CVBackbone(torch.nn.Module):

    def __init__(self, cv_type, output_type, diffaug=True, device='cpu'):
        super().__init__(
        )
        ...
        self.class_name_dict = {
                ...
                'convnext': 'vision_aided_loss.cvmodel.ConvNext',
                'face_normals': 'vision_aided_loss.face_normals.Normals',
                'zcx': 'torch.nn.Identity',
            }
